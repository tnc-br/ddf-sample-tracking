...
export default function ImportSamples() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (
      cancelContainer?.contains(event.target) ||
      greatContainer?.contains(event.target)
    ) {
      handleCloseBarClick()
      return
    }
          ...
          <div className="import-status-actions">
            ...
            >
              <div className="import-status-button-slate-layer">
                <div className="import-status-button-text">{t('great')}</div>
              </div>
            </div>
          </div>
        </div>
  ...
  function onImportErrorBar() {
    return (
      <div
        id="import-error-bar"
        className="import-success-status-wrapper error-background-color"
      >
        <div className="import-status-icon-wrapper">
          <div className="import-status-icon">
        ...
        <div className="import-status-text-wrapper">
          <div className="import-status-text">{t('fileNotUploadedErrors')}</div>
        ...
        <div className="import-status-actions-wrapper">
          <div className="import-status-actions">
            <div
              id="import-error-cancel"
              className="import-cancel-button pointer"
            >
              <div className="import-status-button-slate-layer">
                <div className="import-cancel-button-text">{t('cancel')}</div>
              </div>
            </div>
            <div
              id="import-error-download"
              className="import-error-status-button icon-color-red pointer"
            >
              <div className="import-status-button-slate-layer">
                <div className="import-status-button-text">{t('download')}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    Papa.parse(event.target.files[0], {
      ...
      complete: async function (results) {
        ...
        if (foundErrors) {
          ...
          const statusBarWrapper = document.getElementById('import-status-bar')
          const errorBar =
            document.getElementById('import-status-bars')!.firstChild
        ...
        batch
          ...
          .then(async () => {
            ...
            const statusBarWrapper =
              document.getElementById('import-status-bar')
            const errorBar =
              document.getElementById('import-status-bars')!.lastChild
            if (statusBarWrapper && errorBar) {
              statusBarWrapper.appendChild(errorBar)
            }
          })
  ...
  return (
    <div>
      ...
      />
      <label htmlFor="fileInput">{t('importSamples')}</label>
